...
img_save_path=osp.join('./results','result_img','%s')
img_vision_path=osp.join('./results','result_vision','%s')
ids = list()
...
for img_id in ids:
    ...
    img_J=Image.open(osp.join(image_path,img_id))
    img_source = transform_norm(img_J)
    img_source=torch.unsqueeze(img_source.cuda(),0)
    st=time.time()
    # call the generator to extract the target image and the mask
